...
solar_constant = 1320.0 #(1.0 - 0.0) * planets[planet]['Stellar Luminosity']   / (4. * np.pi * (planets[planet]['Semi-major axis'])**2)   # (1.0 - 0.0) * L_Star / (4. * np.pi * a_b**2) = (1.0 - 0.0) * 0.00073*3.828e26 / (4. * np.pi * (0.0259*AU)**2) for Teegarden b
# Surface pressure [Pa]
p_surf = 1e5
...
exp = Experiment('2_1320_as007', codebase=cb)
exp.clear_rundir()
...
diag.add_file('atmos_monthly', 1, 'seconds', time_units='seconds')
...
diag.add_field('atmosphere', 'dt_tg_convection', time_avg=False) # due to convection
diag.add_field('atmosphere', 'dt_tg_condensation', time_avg=False) # due to large-scale condensation
...
diag.add_field('socrates', 'soc_flux_lw_down', time_avg=False)
diag.add_field('socrates', 'soc_flux_sw_up', time_avg=False)
diag.add_field('socrates', 'soc_flux_sw_down', time_avg=False)
diag.add_field('socrates', 'soc_flux_direct', time_avg=False)

#diag.add_field('socrates', 'soc_cff', time_avg=True)

diag.add_field('socrates', 'soc_spectral_olr', time_avg=False)
diag.add_field('socrates', 'soc_spectral_asr', time_avg=False)
#diag.add_field('socrates', 'soc_spectral_surf_sw_down', time_avg=True)

#clear sky fluxes
diag.add_field('socrates', 'soc_surf_flux_lw_clr', time_avg=False)
diag.add_field('socrates', 'soc_surf_flux_sw_clr', time_avg=False)
diag.add_field('socrates', 'soc_surf_flux_lw_down_clr', time_avg=False)
diag.add_field('socrates', 'soc_surf_flux_sw_down_clr', time_avg=False)
diag.add_field('socrates', 'soc_olr_clr', time_avg=False)
diag.add_field('socrates', 'soc_toa_sw_clr', time_avg=False) 
diag.add_field('socrates', 'soc_toa_sw_down_clr', time_avg=False) 
diag.add_field('socrates', 'soc_flux_lw_up_clr', time_avg=False)
diag.add_field('socrates', 'soc_flux_lw_down_clr', time_avg=False)
diag.add_field('socrates', 'soc_flux_sw_up_clr', time_avg=False)
diag.add_field('socrates', 'soc_flux_sw_down_clr', time_avg=False)
diag.add_field('socrates', 'soc_flux_direct_clr', time_avg=False)

#diag.add_field('cloud_simple', 'cf', time_avg=False)
#diag.add_field('cloud_simple', 'reff_rad', time_avg=False)
#diag.add_field('cloud_simple', 'frac_liq', time_avg=False)
#diag.add_field('cloud_simple', 'qcl_rad', time_avg=False)
#diag.add_field('cloud_simple', 'simple_rhcrit', time_avg=False)
#diag.add_field('cloud_simple', 'rh_min', time_avg=False)
#diag.add_field('cloud_simple', 'rh_in_cf', time_avg=False)
...
exp.namelist = namelist = Namelist({
    ...
    'socrates_rad_nml': {
        ...
        'lw_spectral_filename':"/proj/bolinc/users/x_ryabo/socrates_edited_for_isca/spectral_files_for_GCMs/Teegarden/miniSuran_lw.sf",
        'sw_spectral_filename':"/proj/bolinc/users/x_ryabo/socrates_edited_for_isca/spectral_files_for_GCMs/Teegarden/miniSuran_sw.sf",
        'do_read_ozone': True,
    ...
    'idealized_moist_phys_nml': {
        ...
        'convection_scheme': 'None', #'SIMPLE_BETTS_MILLER', #Use simple Betts miller convection            
    ...
    'large_scale_cloud_nml': { 
        'do_freezedry':True,
        'cf_diag_formula_name':'sundqvist'
    },

    'vert_turb_driver_nml': {
        'do_mellor_yamada': False,     # default: True
        'do_diffusivity': False#True         # default: False
    },
    ...
    'mixed_layer_nml': {
        ...
        'albedo_value': 0.07,                  #Albedo value used      
    },
    ...
    'damping_driver_nml': {
        ...
        'sponge_pbottom':  20.0*(p_surf/1e5), #150., #Setting the lower pressure boundary for the model sponge layer in Pa.
...
if __name__=="__main__":
        ...
        restart_files = '/proj/bolinc/users/x_ryabo/Isca-Ryan_outputs/2_1320_as007/restarts/gridpoint000/res0273.tar.gz'
        
        exp.run(274, restart_file=restart_files, use_restart=True, num_cores=NCORES, overwrite_data=overwrite)

        #for i in range(142,499): # 60 years + 1 year: to 720 with 16 bands, then to 732 with 400 bands
