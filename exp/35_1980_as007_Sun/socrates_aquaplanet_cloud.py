...
solar_constant = 1980.0 #(1.0 - 0.0) * planets[planet]['Stellar Luminosity']   / (4. * np.pi * (planets[planet]['Semi-major axis'])**2)   # (1.0 - 0.0) * L_Star / (4. * np.pi * a_b**2) = (1.0 - 0.0) * 0.00073*3.828e26 / (4. * np.pi * (0.0259*AU)**2) for Teegarden b
# Surface pressure [Pa]
p_surf = 1e5
...
exp = Experiment('35_1980_as007_Sun', codebase=cb)
exp.clear_rundir()
...
if __name__=="__main__":
        ...
        restart_files = '/proj/bolinc/users/x_ryabo/Isca-Ryan_outputs/35_1980_as007_Sun/run0247/res0247/*'

        exp.run(248, use_restart=restart_files, num_cores=NCORES, overwrite_data=overwrite)

        for i in range(249,499): # 60 years + 1 year: to 720 with 16 bands, then to 732 with 400 bands
