...
L_Star = L_Sun #planets[planet]['Stellar Luminosity']    
# Radius [m]
radius = planets[planet]['Radius']     
# Gravity [m.s^-2]
grav = planets[planet]['Gravity']
# Semi-major axis [m]
a_major = 0.9586020072058989*AU #planets[planet]['Semi-major axis'] # corresponds to a stellar constant of 1481 W/m2
# Angular velocity [rad.s^-1]
omega = angular_velocity(1.0) #planets[planet]['Angular velocity']
# Orbital period [s]
orbital_period = 342.8127815331184 * 86400. # corresponds to a stellar constant of 1481 W/m2
...
solar_constant = 1481.0 #(1.0 - 0.0) * planets[planet]['Stellar Luminosity']   / (4. * np.pi * (planets[planet]['Semi-major axis'])**2)   # (1.0 - 0.0) * L_Star / (4. * np.pi * a_b**2) = (1.0 - 0.0) * 0.00073*3.828e26 / (4. * np.pi * (0.0259*AU)**2) for Teegarden b
# Surface pressure [Pa]
p_surf = 1e5
...
exp = Experiment('presentday_1481_Sun_rot', codebase=cb)
exp.clear_rundir()
...
exp.namelist = namelist = Namelist({
    ...
    'socrates_rad_nml': {
        ...
        'lw_spectral_filename':"/proj/bolinc/users/x_ryabo/socrates_edited_for_isca/spectral_files_for_GCMs/Sun/Suran_lw.sf",
        'sw_spectral_filename':"/proj/bolinc/users/x_ryabo/socrates_edited_for_isca/spectral_files_for_GCMs/Sun/Suran_sw.sf",
        'do_read_ozone': True,
    ...
    'damping_driver_nml': {
        ...
        'sponge_pbottom':  150., #20.0*(p_surf/1e5), #150., #Setting the lower pressure boundary for the model sponge layer in Pa.
...
if __name__=="__main__":
        ...
        restart_files = '/proj/bolinc/users/x_ryabo/Isca-Ryan_outputs/presentday_1481_Sun_rot/run0500/res0500/*'

        exp.run(500, use_restart=restart_files, num_cores=NCORES, overwrite_data=overwrite)

        for i in range(501,502): # 60 years + 1 year: to 720 with 16 bands, then to 732 with 400 bands
