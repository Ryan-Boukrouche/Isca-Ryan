...
# ===== EXOMOON PARAMETERS =====
radius_exomoon = 0.25 * radius_Earth
mass_exomoon = mean_density_Earth * (4/3)*np.pi*radius_exomoon**3
orbital_period_exomoon = 3.551181 # orbital period of Europa around Jupiter [days]
a_exomoon = 671.1e6 # semi-major axis around Jupiter [m]
...
planets = { 'Earth': {'Stellar Luminosity': L_Sun, 'Radius': radius_Earth, 'Gravity': grav_Earth, 'Semi-major axis': AU, 'Angular velocity': angular_velocity(1.0)},
            ...
            'Exomoon':   {'Stellar Luminosity': L_Sun, 'Radius': radius_exomoon, 'Gravity': surface_gravity(mass_exomoon/mass_Earth,radius_exomoon/radius_Earth), 'Semi-major axis': a_exomoon, 'Angular velocity': angular_velocity(orbital_period_exomoon)}}
...
# Orbital period [s]
orbital_period = orbital_period_Jupiter * day_s #4.90634 * 86400.
...
solar_constant = 550.0#1361.0 #(1.0 - 0.0) * planets[planet]['Stellar Luminosity']   / (4. * np.pi * (planets[planet]['Semi-major axis'])**2)   # (1.0 - 0.0) * L_Star / (4. * np.pi * a_b**2) = (1.0 - 0.0) * 0.00073*3.828e26 / (4. * np.pi * (0.0259*AU)**2) for Teegarden b
# Surface pressure [Pa]
p_surf = 1e5
...
exp = Experiment('exomoon', codebase=cb)
exp.clear_rundir()
...
exp.namelist = namelist = Namelist({
    ...
    'mixed_layer_nml': {
        'tconst' : 300., # Surface temperature target is 300 K.
    ...
    'damping_driver_nml': {
        ...
        'sponge_pbottom':  150.0*(p_surf/1e5), #150., #Setting the lower pressure boundary for the model sponge layer in Pa.
...
if __name__=="__main__":
        ...
        restart_files = '/proj/bolinc/users/x_ryabo/Isca-Ryan_outputs/exomoon/run0078/res0078/*'

        exp.run(78, use_restart=restart_files, num_cores=NCORES, overwrite_data=overwrite)

        for i in range(79,80): # 60 years + 1 year: to 720 with 16 bands, then to 732 with 400 bands
